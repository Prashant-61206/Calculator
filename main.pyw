...
root.title("CALCULATOR") #the Title of GUI 
root.config(bg="blue")#background of GUI
root.iconbitmap("E:\Saini\Programming\Python\project -1\Calculator\download.ico") # icon of GUI
root.geometry("400x580") #GUI size 
root.maxsize(width=400,height=580) # GUI Maxsize
root.minsize(width=400,height=580)# GUI Minsize

Label(root,text="Made by Prashant",fg= "red").pack() # Creating a Label (Made by Prashant)

entry=Entry(root,width=60,font=150,highlightcolor="red") #Creating a Entry box
entry.pack(padx=50,pady=40)# the position of Entry box
frame=Frame(root,width=300,height=300) #Creating a Frame
frame.config(bg="red")# a background of Frame
frame.pack()

''' Now Creating button and fuction of 1,2,3,4,5,6,7,8,9,0,-,+,=,*,**,/ and clear'''

def bt1():
    entry.insert(END,"1")
button1=Button(frame,width=8,text="1",height=3,command=bt1,font=2,bg="orange")
